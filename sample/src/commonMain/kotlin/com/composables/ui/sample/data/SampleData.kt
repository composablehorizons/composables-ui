...
object Posts {
    ...
    private val replies = listOf(
        ...
        Post("leg-day-reply-stairs", UserProfiles.coachCam.id, "5m", "Do not sit down for too long. The second you stand up tomorrow, negotiations begin.", 1, 12),
        Post("leg-day-reply-pr", UserProfiles.milesLift.id, "4m", "The stair descent after leg day should count as a separate workout.", 3, 26),
        Post("dinner-reply-fries", UserProfiles.jamieOutside.id, "3m", "Round three is where fries for the table become legally required.", 4, 19),
        Post("dinner-reply-photos", UserProfiles.lenaJpg.id, "2m", "Please tell me someone took exactly one clear photo before the lighting got mysterious.", 2, 14),
        Post("warmup-reply-honest", UserProfiles.miaRuns.id, "42m", "I call the walk there my emotional warmup and then act surprised when the bar feels heavy.", 2, 31),
        Post("warmup-reply-science", UserProfiles.milesLift.id, "35m", "The playlist starts before the first set, so technically preparation has begun.", 1, 17),
        Post("fit-reply-jacket", UserProfiles.softLaunch.id, "1h", "The jacket is doing main character work. Documentation was the only responsible choice.", 5, 44),
        Post("fit-reply-mirror", UserProfiles.cityEm.id, "58m", "Apartment elevator lighting finally chose kindness.", 2, 29),
    )

    private val repliesByAuthorId = replies.groupBy { it.authorId }
    private val replyById = replies.associateBy { it.id }
    private val repliesByPostId = mapOf(
