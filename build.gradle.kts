...
subprojects {
  ...
  val injectWasmPreloads by
      tasks.registering {
        ...
        doLast {
          ...
          val preloadBlock =
              (scriptPreloads + wasmPreloads).joinToString(
                  separator = "\n",
                  prefix = "  <!-- wasm-preloads:start -->\n",
                  postfix = "\n  <!-- wasm-preloads:end -->",
              )

          val existingPreloadBlock =
              Regex(
                  pattern =
  ...
  tasks
      ...
      .configureEach { finalizedBy(injectWasmPreloads) }
}
...
fun readDocsSources(file: File): DocsSources {
  ...
  file.readLines().forEachIndexed { index, rawLine ->
    ...
    val indent =
        lineWithoutComment.indexOfFirst { !it.isWhitespace() }.let { if (it == -1) 0 else it }
  ...
  return DocsSources(
      ...
      components =
          DocsSourceSection(
              root = roots.getValue("components"), files = files.getValue("components")),
  )
}
...
val generateDocsDemoSources by
    tasks.registering {
      group = "documentation"
      ...
      doLast {
        ...
        docsDemoSources.values.distinct().forEach { fileName ->
          ...
          check(sourceFile.isFile) { "Missing demo source: ${sourceFile.relativeTo(rootDir)}" }
          val displaySource =
              sourceFile
                  .readText()
...
val generateDocsComponentSources by
    tasks.registering {
      group = "documentation"
      ...
      doLast {
        ...
        docsComponentSources.forEach { (displayName, sourcePath) ->
          ...
          check(sourceFile.isFile) { "Missing component source: ${sourceFile.relativeTo(rootDir)}" }
...
val generateDocsApiReference by
    tasks.registering(Exec::class) {
      ...
      inputs.files(fileTree("ui/src/commonMain/kotlin") { include("**/*.kt") })
...
tasks.register<Sync>("bundleDocs") {
  ...
  val demoDistribution =
      project(":demo").layout.buildDirectory.dir("dist/wasmJs/productionExecutable")

  inputs.file(docsSourcesFile)

  into(bundleDirectory)
  from(docsSource.file("docs.yml"))
  from(generatedDocsPages) { into("pages") }
  from(generatedDemoSources) { into("demo-sources") }
  from(generatedComponentSources) { into("component-sources") }
  from(demoDistribution) { into("apps/composables-ui-demos") }

  doFirst {
    val pages = fileTree(generatedDocsPages) { include("**/*.md") }.files
    val legacyPatterns =
        listOf(
            "```compose",
            "<ApiReference",
  ...
  doLast {
    ...
    val demoManifest =
        docsDemoSources.entries.joinToString(",\n") { (id, fileName) ->
          """
                "$id": {
